...
export class ScriptService {
  ...
  loadScript(): Array<SoundElement> {
    const announcer: string = this.settings.announcer;
    const verbose: boolean = this.settings.verbose;
    const flair: boolean = this.settings.flair;
    const characters = this.settings.characters;
    const array: Array<SoundElement> = new Array<SoundElement>();

    if (announcer === 'en-gb-C' || announcer === 'en-gb-D' || announcer === 'cathy') {
